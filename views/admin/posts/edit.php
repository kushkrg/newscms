<style>
    ...
    .current-image img { max-width: 100%; height: auto; max-height: 160px; border: 1px solid #e5e5e5; }
    .current-image-label { font-size: 0.75rem; color: #666; margin-top: 0.25rem; }
    @media (max-width: 768px) { .form-grid { grid-template-columns: 1fr; } }
</style>

<?php
    $tagNamesList = array_map(function ($t) { return $t['name'] ?? ''; }, $postTags ?? []);
    $tagsValue = implode(', ', $tagNamesList);
?>
...
<form method="POST" action="<?= url('admin/posts/' . (int) $post['id'] . '/update') ?>" enctype="multipart/form-data">
    <?= \App\Core\Csrf::field() ?>
    ...
    <div class="form-grid">
        <div class="form-main">
            <div class="form-group">
                ...
                <input type="text" name="title" id="post-title" required value="<?= h($post['title'] ?? '') ?>">
            </div>

            <div class="form-group">
                <label for="post-slug">Slug</label>
                <input type="text" name="slug" id="post-slug" value="<?= h($post['slug'] ?? '') ?>">
                <span class="note">Auto-generated from title if left empty.</span>
            </div>

            <div class="form-group">
                <label for="editor">Content</label>
                <textarea name="content" id="editor" class="large"><?= h($post['content'] ?? '') ?></textarea>
            </div>

            <div class="form-group">
                <label for="excerpt">Excerpt</label>
                <textarea name="excerpt" id="excerpt" class="small"><?= h($post['excerpt'] ?? '') ?></textarea>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-section-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="meta_title" id="meta_title" value="<?= h($post['meta_title'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label for="meta_description">Meta Description</label>
                        <textarea name="meta_description" id="meta_description" class="small"><?= h($post['meta_description'] ?? '') ?></textarea>
                    </div>
                    <div class="form-group">
                        <label for="canonical_url">Canonical URL</label>
                        <input type="url" name="canonical_url" id="canonical_url" value="<?= h($post['canonical_url'] ?? '') ?>">
                    </div>
                    <div class="form-group">
        ...
        <div class="form-sidebar">
            ...
            <div class="form-group">
                ...
                <input type="text" name="tags" id="tags" value="<?= h($tagsValue) ?>" placeholder="tag1, tag2, tag3">
            ...
            <div class="form-group">
                ...
                <?php if (!empty($post['featured_image'])): ?>
                    <div class="current-image">
                        <img src="<?= upload_url($post['featured_image']) ?>" alt="<?= h($post['featured_image_alt'] ?? 'Featured image') ?>">
            ...
            <div class="form-group">
                ...
                <input type="text" name="featured_image_alt" id="featured_image_alt" value="<?= h($post['featured_image_alt'] ?? '') ?>">
            </div>
            ...
            <div class="file-attach">
                ...
                <?php if (!empty($post['download_file'])): ?>
                    <div class="file-attach__current" id="currentDownload">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                        <span><?= h($post['download_file_name'] ?? basename($post['download_file'])) ?></span>
                        <button type="button" class="file-attach__remove" onclick="document.getElementById('currentDownload').remove(); document.getElementById('remove_download_file').value='1';" title="Remove file">&times;</button>
                    </div>
                    <input type="hidden" name="remove_download_file" id="remove_download_file" value="0">
                <?php endif; ?>
                ...
                <div id="upload-panel" class="file-attach__panel active">
                    <div class="form-group" style="margin-bottom:0">
                        ...
                        <span class="note"><?= !empty($post['download_file']) ? 'Upload new file to replace current.' : 'Allowed: PDF, ZIP, RAR, DOC, XLS, PPT (max 50MB)' ?></span>
                    </div>
                </div>
            ...
            <div class="form-group" style="gap: 0.6rem;">
                ...
                <div class="checkbox-group">
                    <input type="checkbox" name="is_featured" id="is_featured" value="1" <?= !empty($post['is_featured']) ? 'checked' : '' ?>>
                    <label for="is_featured">Featured post</label>
                </div>
                <div class="checkbox-group">
                    <input type="checkbox" name="is_sticky" id="is_sticky" value="1" <?= !empty($post['is_sticky']) ? 'checked' : '' ?>>
                    <label for="is_sticky">Sticky post</label>
                </div>
                <div class="checkbox-group">
                    <input type="checkbox" name="allow_comments" id="allow_comments" value="1" <?= isset($post['allow_comments']) ? ($post['allow_comments'] ? 'checked' : '') : 'checked' ?>>
